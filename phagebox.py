...
pcr_subfig1 = pcr_fig.add_subplot(121)    
pcr_subfig2 = pcr_fig.add_subplot(122)

def animate(i):
    print("working")
    pullData = open("s.txt", "r").readlines()
    xList = []
    yList = []
    for line in pullData:
        if len(line) > 1:
            x, y = line.split(",")
            xList.append(x)
            yList.append(y)
    pcr_subfig2.clear()
    pcr_subfig2.plot(xList, yList)
...
#create_figs()
root = phageBoxGUI()
animation.FuncAnimation(pcr_fig2, animate, interval=10)
root.mainloop()
